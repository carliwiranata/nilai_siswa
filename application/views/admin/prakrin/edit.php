        ...
        <div class="col-lg-6 mx-auto">
            ...
            <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                    <h6 class="m-0 font-weight-bold text-primary">Edit Siswa Prakrin</h6>
                </div>
                <div class="card-body">
                    <form action="<?= base_url('admin/editprakrin/' . $siswa['id_prakrin']) ?>" method="post">

                        <div class="form-group">
                            <label for="exampleInputPassword1">Nama Siswa</label>
                            <input type="text" readonly class="form-control" id="exampleInputPassword1" value="<?= $siswa['nama_siswa'] ?>" placeholder="Password">
                        </div>
                        <div class="form-group">
                            <select class="select2-single form-control" name="id_kelas" id="select2Single">
                                <option value="">Pilih Kelas</option>
                                <?php foreach ($kelas as $k) : ?>
                                    <option value="<?= $k['id_kelas'] ?>" <?= $k['kelas'] == $siswa['kelas'] ? 'selected' : '' ?>><?= $k['kelas'] ?></option>
                                <?php endforeach ?>
                            </select>
                        ...
                        <div class="form-group">
                            <select class="select2-single form-control" name="id_sekolah" id="select2Single1">
                                ...
                                <?php foreach ($sekolah as $s) : ?>
                                    <option value="<?= $s['id_sekolah'] ?>" <?= $s['sekolah'] == $siswa['sekolah'] ? 'selected' : '' ?>><?= $s['sekolah'] ?></option>
                                <?php endforeach ?>
                            </select>
                        ...
                        <div class="form-group">
                            <select class="select2-single form-control" name="id_guru" id="select2Single2">
                                ...
                                <?php foreach ($guru as $g) : ?>
                                    <option value="<?= $g['id_guru'] ?>" <?= $g['nama_guru'] == $siswa['nama_guru'] ? 'selected' : '' ?>><?= $g['nama_guru'] ?></option>
                                <?php endforeach ?>
                            </select>
                        ...
                        <div class="form-group" id="simple-date4">
                            ...
                            <div class="input-daterange input-group">
                                <input type="date" value="<?= $siswa['tgl_masuk'] ?>" class="input-sm form-control" name="tgl_masuk" />

                                <div class="input-group-prepend">
                                    <span class="input-group-text">Sampai</span>
                                </div>
                                <input type="date" value="<?= $siswa['tgl_keluar'] ?>" class="input-sm form-control" name="tgl_keluar" />
                            </div>
                        </div>
